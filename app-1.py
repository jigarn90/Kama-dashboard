...
if st.button("Run Scanner"):
    ...
    for i in range(1, len(df)):
        ...
        buy = (df['Close'].iloc[i-1] < df['KAMA'].iloc[i-1]) & (df['Close'].iloc[i] > df['KAMA'].iloc[i]) & trend_up & trend_strong and trend_up and trend_strong
        sell = (df['Close'].iloc[i-1] > df['KAMA'].iloc[i-1]) & (df['Close'].iloc[i] < df['KAMA'].iloc[i]) & (~trend_up) & trend_strong and not trend_up and trend_strong
