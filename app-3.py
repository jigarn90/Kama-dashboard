...
if st.button("Run Strategy"):
    ...
    for i in range(1, len(df)):
        ...
        kama_i = df['KAMA'].iloc[i]
        kama_prev = df['KAMA'].iloc[i - 1]
        ma_i = df['MA'].iloc[i]
        adx_i = df['ADX'].iloc[i]

        trend_up = close_i > ma_i
        trend_strong = adx_i > 25

        buy = (close_prev < kama_prev) and (close_i > kama_i) and trend_up and trend_strong
        sell = (close_prev > kama_prev) and (close_i < kama_i) and (not trend_up) and trend_strong

        if buy:
            buy_signals.append((df.index[i], close_i))
            send_telegram_message(f"📈 BUY @ ₹{close_i:.2f} | TP: ₹{close_i * (1 + take_profit):.2f} | SL Trail: ₹{close_i * (1 - trailing_sl):.2f}")
        elif sell:
            sell_signals.append((df.index[i], close_i))
            send_telegram_message(f"📉 SELL @ ₹{close_i:.2f} | TP: ₹{close_i * (1 - take_profit):.2f} | SL Trail: ₹{close_i * (1 + trailing_sl):.2f}")
