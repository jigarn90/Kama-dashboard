...
def calculate_kama(series, length, fast_ema, slow_ema):
    ...
    volatility = pd.Series([abs(series.shift(i) - series.shift(i + 1)) for i in range(length)]).sum()
...
if st.button("Run Strategy"):
    ...
    for i in range(1, len(df)):
        try:
            ...
            trend_up = bool(close_i > ma_i)
            trend_strong = bool(adx_i > 25)
            ...
            if buy:
                ...
                send_telegram_message(f"📍 BUY Signal: {symbol}\nPrice: ₹{close_i:.2f}\nTP: ₹{close_i * (1 + take_profit):.2f}\nTrailing SL: ₹{close_i * (1 - trailing_sl):.2f}")
            elif sell:
