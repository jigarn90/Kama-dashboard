...
def calculate_kama(df, kama_len=10, fast_ema=2, slow_ema=30):
    ...
    volatility = sum(abs(close.diff(1)).fillna(0).rolling(kama_len).sum())
...
def scan_stock(symbol):
    ...
    # Check for valid values (non-NaN)
    if df[['Close', 'KAMA', 'MA']].iloc[-1].isnull().any() or df[['Close', 'KAMA']].iloc[-2].isnull().any():
        return None

    if (
        df['Close'].iloc[-1] > df['MA'].iloc[-1] and
        df['Close'].iloc[-2] < df['KAMA'].iloc[-2] and
        df['Close'].iloc[-1] > df['KAMA'].iloc[-1]
    ):
